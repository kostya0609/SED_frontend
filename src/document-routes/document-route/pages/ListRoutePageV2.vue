	...
	<Preloader :loading="loading">
		...
		>
			<el-breadcrumb-item
				...
				:key="item.id"
				:to="{ path: `/sed/admin/document-routes/list-v2/${item.id || ''}` }"
			>{{ item.title }}</el-breadcrumb-item>
		</el-breadcrumb>

		<div class="list-route-page">

			<div
				v-if="showPartition"
				class="list-route-page__tree"
			>
				<el-scrollbar>
					<PartitionTree
						:data="partitionsTree"
			...
			<div class="list-route-page__table">
				<BsiTable
				...
				>
					...
					<BsiTableColumn
						...
						v-slot="{ row }"
					>
						{{ row.type == 'partition' ? row.partition_id : row.route_id }}
					...
					>
						<el-link
							type="primary"
							:underline="false"
							@click="goTo(row.partition_id, row.route_id, row.type)"
						>
							<el-icon
...
import { usePartitionTree, PartitionModal, PartitionTree } from "@document-routes/document-route/entities/partition";
...
const { user } = useUser();
const { partitionsTree, breadcrumbs, initPartitionsTree, loading, deletePartition, getBreadcrumbs } = usePartitionTree();
...
const clickPartition = async (partition) => {
	if(partition.id != route.params.partition_id)
		await router.push({ path: `/sed/admin/document-routes/list-v2/${partition.id}`, force: true });
};
...
const handleChangeSettings = async ({ paginate, filter, sort, search }) => {
	return await DocumentRouteRepo.getRouteByParentId({
		paginate,
		sort,
		filter,
		search,
...
const goTo = async (partition_id, route_id, type) => {

	if (type == 'partition') {
		await router.push({ path: `/sed/admin/document-routes/list-v2/${partition_id}`, force: true });
	}

	if (type == 'route') router.push({ name: 'detailDocumentRoutePage', params: { id: route_id } });
};
...
onMounted(() => {
	...
	addButton({
		...
		icon: 'Plus',
		onClick: () => {
			router.push({ path: `/sed/admin/document-routes/create/${route.params.partition_id}` });
		}
	});
	addButton({
		icon: 'Back',
...
.list-route-page ::v-deep(.bsi-table__footer) {
	display: none;
}

.list-route-page ::v-deep(.filter-button) {
	display: none;
}
</style>
