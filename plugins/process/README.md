...
```
git submodule add https://gitlab.bitrix.bsi.local/common/plugin_process.git ./plugins/process
```
...
| ------------------------- | ----------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| created                   | Создан                                          | Вызывается в момент создания БП, когда мы нажимаем кнопку "Принудительно создать процесс"                                                                                        |
| deleted                   | Удален                                          | Вызывается в момент удаления БП, когда мы нажимаем кнопку "Удалить процесс"                                                                                                                   |
| runned                    | Запущен                                        | Вызывается в моментзапуска БП, когда нажимаем на кнопку "Запустить процесс"                                                                                                                  |
| cancelled                 | Аннулирован                                | Вызывается в момент аннулирования процесса, к примеру, когда было принято отрицательное решение или принудительно аннулировали процесс |
| decided                   | Принято решение                         | Вызываеся в момент принятия решения участником                                                                                                                                                                     |
| completed                 | Выполнен                                      | Вызывается когда процесс был выполнен                                                                                                                                                                                      |
| participant-count-changed | Кол-во участников изменилось | Вызывается когда меняется количества участников в бизнес-процессе                                                                                                                                |
...
const onDecided = async (action) => {
  ...
  if (action.template_action_id === TEMPLATE_ACTION.MAKE_COMMENT_AND_NOTIFY_PROCESS_EXECUTOR) {
