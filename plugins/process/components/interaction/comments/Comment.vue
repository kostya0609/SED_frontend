...
const actions = computed(() => props.comment.actions.filter(action => {
	if (
		process.value &&
		process.value.id !== props.comment.process_id ||
		(process.value.status.id == PROCESS_STATUS.PREPARATION || process.value.status.id === PROCESS_STATUS.DONE)
	) {
		return false;
	}

	const participant = props.comment.participant;
	...
	} else if (action.who_access_id === PARTICIPANT_TYPE.EMPLOYEE) {

		if (!participant.action) {
			console.error('Не было найдено действие участника!');
			return false;
		}

		if (!participant.action.extra_fields) {
			return false;
		}

		if (!participant.action.extra_fields.participants) {
			console.error("Не заданы участники действия!");
			return false;
		}

		const users = participant.action.extra_fields.participants.map(item => item.user_id);

		return users.includes(userId.value);
	}
	else {
		return false;
	}
